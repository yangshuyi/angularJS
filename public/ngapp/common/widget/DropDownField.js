angular
    ...
    .directive("dropDownField", ['codeTable', '$q', '$timeout', function(codeTable, $q, $timeout) {
        return {
            ...
            replace : false,
            template:
            '<ui-select ng-model="model.selected" ng-change="changeFunc(model.selected)" theme="select2" append-to-body="true" search-enabled="{{searchEnabled}}" ng-disabled="ngDisabled"' +
            ...
            link : function($scope, $elem, attrs, ngModelCtrl) {
                ...
                function setTotalOptions(array) {
                    _.remove($scope.totalOptions, function() {return true;});
                    ...
                    $q.when(confirmFunc(item.value, true))
                        .then(function (confirmed) {
                            if (confirmed) {
                                ...
                                $timeout(function(){$elem.trigger('change');}, 0); // trigger change event, therefore, trigger the validation process
                            }
                            else {
                                changeSelected(lastSelectedValue);
                            }
